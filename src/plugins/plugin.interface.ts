...
/**
 * Output returned by the generation stage.
 * Used in POST_GENERATE hook to modify the final output.
 */
export interface GenerationOutput {
  /** The output format (e.g., "pdf", "html") */
  format: string;
  /** The generated content (HTML string before PDF conversion) */
  content?: string;
...
export interface IRepo2PDFPlugin {
   ...
   * Common uses: adding metadata, modifying highlighted HTML,
   * aggregating statistics.
   *
   * @param processedFiles - Array of processed files with highlighted content
   * @param config - The current configuration
   ...
   *   // Add line count metadata to each file
   *   return files.map(file => ({
   *     ...file,
   *     metadata: {
   *       ...file.metadata,
   *       lineCount: file.processedContent.split('\n').length
   *     }
   *   }));
   * };
   * ```
   ...
   *
   * @description
   * Use this hook to modify the generated output or perform post-processing.
   * Common uses: adding watermarks, modifying HTML before PDF conversion,
   * logging statistics, sending notifications.
   *
   * @param output - The generation output containing format and content
   * @param config - The current configuration
   * @returns Modified output or undefined to keep original
   *
   * @example
   * ```typescript
   * [HookPoint.POST_GENERATE] = (output: GenerationOutput): GenerationOutput => {
   *   if (output.content) {
   *     // Add a watermark to HTML
   *     output.content = output.content.replace(
   *       '</body>',
   *       '<div class="watermark">Generated by repo2pdf</div></body>'
   *     );
   *   }
   *   return output;
   * };
